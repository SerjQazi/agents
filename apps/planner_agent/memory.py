...
def seed_default_memory(storage: Storage, config: PlannerConfig) -> None:
    for key, value in DEFAULT_RULES.items():
        storage.upsert_memory_note("rule", key, value, "planner-agent-default")

    for key, value in DEFAULT_MAPPINGS.items():
        storage.upsert_memory_note("mapping", key, value, "planner-agent-default")

    storage.upsert_memory_note("path", "agents_repo", str(config.agents_root), "planner-agent-default")
    storage.upsert_memory_note("path", "incoming_scripts", str(config.incoming_dir), "planner-agent-default")
    storage.upsert_memory_note("path", "server_resources", str(config.server_resources), "planner-agent-default")
    storage.upsert_memory_note("path", "reports", str(config.reports_dir), "planner-agent-default")
