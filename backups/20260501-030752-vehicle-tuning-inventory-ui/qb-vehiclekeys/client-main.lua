...
local looped = false
local parkedVehicleLockStates = {}

-- AGENT FIX START: config-driven NPC/world lock decisions
local function shouldLockDrivingNpcVehicle()
    if Config.DrivingNpcVehicleLocked ~= nil then return Config.DrivingNpcVehicleLocked end
    return Config.LockNPCDrivingCars
end

local function shouldLockParkedNpcVehicle(vehicle)
    if parkedVehicleLockStates[vehicle] ~= nil then
        return parkedVehicleLockStates[vehicle]
    end

    if Config.ParkedVehicleLockChance ~= nil then
        parkedVehicleLockStates[vehicle] = math.random(100) <= Config.ParkedVehicleLockChance
        return parkedVehicleLockStates[vehicle]
    end

    parkedVehicleLockStates[vehicle] = Config.LockNPCParkedCars
    return parkedVehicleLockStates[vehicle]
end

local function requiresAdvancedLockpick(vehicle)
    local advancedClasses = Config.AdvancedLockpickClasses or {}
    return advancedClasses[GetVehicleClass(vehicle)] == true
end
-- AGENT FIX END
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            if entering ~= 0 and not isBlacklistedVehicle(entering) then
                ...
                if driver ~= 0 and not IsPedAPlayer(driver) and not HasKeys(plate) and not carIsImmune then
                    ...
                    elseif shouldLockDrivingNpcVehicle() then
                ...
                elseif driver == 0 and entering ~= lastPickedVehicle and not HasKeys(plate) and not isTakingKeys then
                    QBCore.Functions.TriggerCallback('qb-vehiclekeys:server:checkPlayerOwned', function(playerOwned)
                        if not playerOwned then
                            if shouldLockParkedNpcVehicle(entering) then
                                TriggerServerEvent('qb-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 2)
                            else
                                TriggerServerEvent('qb-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 1)
                            end
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
    ...
    -- AGENT FIX START: require advanced lockpick for configured high-end classes
    if requiresAdvancedLockpick(vehicle) and not isAdvanced then
        QBCore.Functions.Notify('This vehicle requires an advanced lockpick.', 'error')
        return
    end
    -- AGENT FIX END
