const InventoryContainer = Vue.createApp({
    ...
    computed: {
        ...
        pocketSlots() {
            return this.buildSlotRange(1, Math.min(this.totalSlots, 5));
        },
        carrySlots() {
            return this.buildSlotRange(6, Math.min(this.totalSlots, 20));
        },
        backpackSlots() {
            return this.buildSlotRange(21, this.totalSlots);
        },
        isBackpackVisuallyLocked() {
            return !this.backpackEquipped;
        },
    },
    watch: {
    ...
    methods: {
        ...
        openInventory(data) {
            ...
            this.backpackEquipped = Boolean(data.backpackEquipped);
        ...
        buildSlotRange(start, end) {
            const firstSlot = Number(start);
            const lastSlot = Number(end);

            if (!Number.isFinite(firstSlot) || !Number.isFinite(lastSlot) || lastSlot < firstSlot) {
                return [];
            }

            return Array.from({ length: lastSlot - firstSlot + 1 }, (_, index) => firstSlot + index);
        },
        // AGENT FIX START: reject visual-only character drops before ground fallback
        isCursorOver(elements, selector) {
            return elements.some((el) => typeof el.closest === "function" && el.closest(selector));
        },
        // AGENT FIX END
        ...
        endDrag(event) {
            ...
            } else if (this.isCursorOver(elementsUnderCursor, ".visual-only-drop-zone")) {
                this.inventoryError(this.currentlyDraggingSlot);
            } else if (this.isOtherInventoryEmpty && this.dragStartInventoryType === "player") {
                const isOverGroundDropZone = this.isCursorOver(elementsUnderCursor, ".ground-drop-zone");
                const isOverInventoryGrid = this.isCursorOver(elementsUnderCursor, ".inventory-grid, .item-grid");
                const isOverInventoryShell = this.isCursorOver(elementsUnderCursor, ".inventory-shell");

                if (isOverGroundDropZone || (!isOverInventoryGrid && !isOverInventoryShell)) {
        ...
        inventoryError(slot) {
            const inventorySelector = this.dragStartInventoryType === "other" ? ".other-inventory-section" : ".player-inventory-section";
            let slotElements = Array.from(document.querySelectorAll(`${inventorySelector} .item-slot[data-slot="${slot}"]`));
            if (slotElements.length === 0) {
                slotElements = Array.from(document.querySelectorAll(`.item-slot[data-slot="${slot}"]`));
            }
        ...
        // AGENT FIX START: temporary browser-only preview mode
        isBrowserPreviewMode() {
            const params = new URLSearchParams(window.location.search);
            return params.get("preview") === "1" || typeof window.invokeNative !== "function";
        },
        createPreviewItem(slot, name, label, amount, weight, image, extra = {}) {
            return {
                slot,
                name,
                label,
                ...
                weight,
                image,
                type: extra.type || "item",
                unique: Boolean(extra.unique),
                useable: extra.useable !== false,
                shouldClose: false,
                info: {
                    description: extra.description || `${label} preview item`,
                    quality: extra.quality === undefined ? 100 : extra.quality,
                    ...(extra.info || {}),
                },
            };
        },
        getPreviewInventoryData() {
            return {
                action: "open",
                inventoryLabel: "Preview Inventory",
                slots: 35,
                maxweight: 120000,
                inventory: [
                    this.createPreviewItem(1, "weapon_pistol", "Pistol", 1, 1000, "weapon_pistol.png", {
                        type: "weapon",
                        unique: true,
                        quality: 82,
                        description: "Preview sidearm with durability data",
                        info: { serie: "PREVIEW-0421", ammo: 12 },
                    }),
                    this.createPreviewItem(2, "bandage", "Bandage", 8, 100, "bandage.png", { quality: 100 }),
                    this.createPreviewItem(3, "radio", "Radio", 1, 1000, "radio.png", { unique: true }),
                    this.createPreviewItem(4, "phone", "Phone", 1, 700, "phone.png", { unique: true }),
                    this.createPreviewItem(5, "armor", "Armor", 2, 5000, "armor.png", { quality: 65 }),
                    this.createPreviewItem(7, "sandwich", "Sandwich", 4, 200, "sandwich.png"),
                    this.createPreviewItem(9, "water_bottle", "Water", 6, 500, "water_bottle.png"),
                    this.createPreviewItem(12, "lockpick", "Lockpick", 3, 300, "lockpick.png", { quality: 45 }),
                    this.createPreviewItem(18, "joint", "Joint", 2, 100, "joint.png"),
                    this.createPreviewItem(24, "goldchain", "Gold Chain", 1, 1500, "goldchain.png", { unique: true }),
                    this.createPreviewItem(31, "repairkit", "Repair Kit", 1, 2500, "repairkit.png", { quality: 88 }),
                ],
                other: {
                    name: "drop-preview",
                    label: "Ground Preview",
                    maxweight: 40000,
                    slots: 24,
                    inventory: [
                        this.createPreviewItem(1, "cash", "Marked Bills", 3, 0, "markedbills.png", {
                            description: "Mock ground drop item",
                            info: { worth: 1250 },
                        }),
                        this.createPreviewItem(2, "pistol_ammo", "Pistol Ammo", 4, 1000, "pistol_ammo.png"),
                        this.createPreviewItem(5, "advancedlockpick", "Advanced Lockpick", 1, 500, "advancedlockpick.png", { quality: 72 }),
                        this.createPreviewItem(9, "firstaid", "First Aid", 2, 2500, "firstaid.png"),
                    ],
                },
            };
        },
        enableBrowserPreview() {
            if (!this.isBrowserPreviewMode()) {
                return;
            }
            this.openInventory(this.getPreviewInventoryData());
            this.inventoryLabel = "Preview Inventory";
            this.showHotbar = true;
            this.hotbarItems = [1, 2, 3, 4, 5].map((slot) => this.playerInventory[slot] || null);
        },
        // AGENT FIX END
    },
    mounted() {
